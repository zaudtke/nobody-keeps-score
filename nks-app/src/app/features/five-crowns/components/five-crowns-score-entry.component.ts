...
import type { RoundDef } from '../five-crowns.model';

export interface FiveCrownsScoreEntryResult {
  roundScore: number;
  newTotal: number;
}

@Component({
  selector: 'app-five-crowns-score-entry',
  standalone: true,
      ...
      >
        <div>
          ...
          <div class="flex items-center gap-2 mt-[2px]">
            ...
            <span
              class="text-[0.55rem] font-semibold tracking-[0.04em] uppercase px-1.5 py-[1px] rounded-[4px]
                     bg-ink-100 text-ink-500 dark:bg-[rgba(94,100,116,0.15)] dark:text-ink-400
                     border border-ink-200 dark:border-[rgba(94,100,116,0.2)]"
            >
              Round {{ roundDef().label }} · wild {{ roundDef().wild }}
            </span>
          </div>
        </div>
        <button
      ...
      <div class="px-4 pt-3 pb-4 flex flex-col gap-3">
        ...
        >
          ...
          <span
            ...
            [class]="roundScoreValue() !== null && roundScoreValue()! < 0
              ? 'text-ruby-500 dark:text-ruby-400'
              : 'text-felt-600 dark:text-felt-400'"
          >
            {{ rawScore() === '' ? '—' : roundScoreValue() }}
          </span>
          <span class="text-ink-500 dark:text-ink-700 text-[0.7rem]">=</span>
        ...
        <!-- Negative warning -->
        @if (roundScoreValue() !== null && roundScoreValue()! < 0) {
          <p class="text-[0.65rem] text-ruby-500 dark:text-ruby-400 text-center -mt-1">
            Negative score — house-rule override
          </p>
        }
...
export class FiveCrownsScoreEntryComponent {
  ...
  protected confirm(): void {
    const sv = this.roundScoreValue();
    if (sv === null) return;
    this.confirmed.emit({ roundScore: sv, newTotal: this.liveTotal() });
  }
}
