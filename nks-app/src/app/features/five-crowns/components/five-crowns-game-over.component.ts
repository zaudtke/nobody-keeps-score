...
import type { FiveCrownsGameOverResult } from '../five-crowns.model';

@Component({
  selector: 'app-five-crowns-game-over',
  standalone: true,
  ...
  template: `
    ...
    >
      ...
      <p class="text-[0.75rem] text-felt-400 mt-1">
        @if (result().isTie) {
          Both scored {{ result().winnerScore }} — tiebreaker at the table
        } @else {
          Lowest score wins · finished with {{ result().winnerScore }}
        }
      </p>
      <p class="text-[0.65rem] text-ink-500 mt-2 italic">Well played.</p>
    </div>
        ...
        @for (row of result().standings; track row.playerId) {
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              @if (row.lastRoundScore !== null) {
                <p class="text-[0.63rem] text-ink-400 dark:text-ink-600 mt-[1px]">
                  +{{ row.lastRoundScore }} in round K
                </p>
              }
              @if (row.total < 0) {
...
export class FiveCrownsGameOverComponent {
  result = input.required<FiveCrownsGameOverResult>();
