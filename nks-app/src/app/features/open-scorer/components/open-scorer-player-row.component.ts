...
import type { OpenScorerPlayerStanding } from '../open-scorer.model';

@Component({
  selector: 'app-open-scorer-player-row',
  standalone: true,
      ...
      <div class="flex-1 min-w-0">
        ...
        @if (standing().roundScore !== null) {
          <p class="text-[0.65rem] text-ink-400 dark:text-ink-600 mt-[2px]">
            {{ standing().roundScore! >= 0 ? '+' : '' }}{{ standing().roundScore }} this round
          </p>
        }
      </div>
...
export class OpenScorerPlayerRowComponent {
  standing = input.required<OpenScorerPlayerStanding>();
