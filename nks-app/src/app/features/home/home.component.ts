    ...
    <div class="flex-1 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden pb-5">
      ...
      @if (isAuthenticated()) {
        ...
        <div class="relative overflow-hidden px-4.5 pt-5 pb-4">
          ...
          <div class="text-[0.68rem] font-medium tracking-[0.06em] uppercase mb-0.5
                      text-felt-600 dark:text-felt-400">
            {{ greeting() }}
          </div>
          <div class="font-display text-[1.45rem] font-bold leading-[1.05] tracking-[-0.02em] relative
                      text-ink-900 dark:text-cream-100">
            {{ hostName() }}
          </div>
          <div class="absolute right-4 top-1/2 -translate-y-1/2 text-[1.6rem] leading-none tracking-[-0.05em] pointer-events-none select-none
        ...
        <div class="px-3.5 pt-1">
          ...
          <p class="text-center text-[0.63rem] mt-1.75
        ...
        <div class="px-3.5 pt-4">
          <div class="text-[0.6rem] font-semibold tracking-[0.09em] uppercase mb-2
                      text-ink-400 dark:text-ink-600">
            Available Games
          </div>
          <div class="flex flex-wrap gap-1.25">
            @for (game of games; track game.name) {
              <span class="px-2.25 py-1 rounded-full text-[0.65rem] font-medium flex items-center gap-1
        ...
        <div class="relative overflow-hidden px-5 pt-7 pb-5.5 text-center">
          ...
          <div class="absolute inset-0 pointer-events-none
                      ...
                      hidden dark:block"></div>
          <div class="text-[1.1rem] tracking-[0.18em] mb-2.5 relative
                      text-[rgba(26,127,75,0.35)] dark:text-[rgba(71,184,122,0.45)]">
            ♠ ♥ ♦ ♣
          </div>
          <h1 class="font-display text-[1.55rem] font-bold leading-[1.05] tracking-[-0.025em] mb-1.5 relative
                     text-ink-900 dark:text-cream-100">
            Nobody's<br>Keeping <em class="not-italic font-normal text-gold-400">Score</em>
          </h1>
          <p class="text-[0.72rem] font-light tracking-[0.03em] relative
        ...
        <div class="px-4.5 mb-3.5 text-center">
          <p class="text-[0.75rem] leading-[1.55]
                    text-ink-500 dark:text-ink-400">
            A personal scorekeeper for card game nights — fast to start, simple to track, satisfying to settle.
          </p>
        </div>

        <!-- Supported games grid -->
        <div class="px-3.5 mb-4">
          <div class="text-[0.62rem] font-semibold tracking-[0.09em] uppercase mb-2 text-center
                      text-ink-400 dark:text-ink-600">
            Supported Games
          </div>
          <div class="grid grid-cols-2 gap-1.5">
            @for (game of gameChips; track game.name) {
              <div class="rounded-[10px] p-[9px_10px] flex items-center gap-1.75
                ...
                <div>
                  <span class="block text-[0.72rem] font-semibold leading-[1.2]
                               text-ink-700 dark:text-ink-200">{{ game.name }}</span>
                  <span class="block text-[0.58rem] font-normal leading-none mt-px
                               text-ink-400 dark:text-ink-600">{{ game.tag }}</span>
                </div>
              </div>
            }
          </div>
        ...
        <!-- Hairline -->
        <div class="h-px mx-4 mb-4
                    bg-cream-200 dark:bg-[rgba(255,255,255,0.06)]"></div>
        ...
        <div class="px-3.5">
          ...
          <p class="text-center text-[0.62rem] mt-1.75
                    text-ink-400 dark:text-ink-700">
