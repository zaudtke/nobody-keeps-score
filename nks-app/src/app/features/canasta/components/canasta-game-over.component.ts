...
import type { CanastaGameOverResult } from '../canasta.model';

@Component({
  selector: 'app-canasta-game-over',
  standalone: true,
  imports: [DecimalPipe],
  ...
  template: `
    ...
    >
      ...
      <p class="text-[0.75rem] text-felt-400 mt-1">
        @if (result().isTie) {
          Both reached {{ result().winnerScore | number }} — tiebreaker at the table
        } @else {
          Finished with {{ result().winnerScore | number }} pts
        }
      </p>
    </div>
    ...
    >
      ...
      <div class="flex flex-col gap-2">
        @for (row of result().standings; track row.playerId) {
          ...
          >
            <!-- Place -->
            <span class="font-display text-[1rem] font-bold w-5 text-center shrink-0" [class]="placeClasses(row.place)">
              {{ row.place }}
            </span>
            ...
            <div class="flex-1 min-w-0">
              ...
              @if (row.total < 0) {
                <p class="text-[0.65rem] text-ruby-500 dark:text-ruby-400 mt-[1px]">
                  Finished negative
                </p>
              }
            </div>
...
export class CanastaGameOverComponent {
  result = input.required<CanastaGameOverResult>();
